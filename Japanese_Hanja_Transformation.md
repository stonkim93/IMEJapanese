...
## 5. 성능 및 메모리 최적화 (2026-09-24 업데이트)

한자 변환은 입력 속도와 밀접하게 연관되어 있으므로 다음과 같은 고강도 최적화가 적용되었습니다.

### 5.1 Viterbi / Beam Search 알고리즘 최적화 (GC 부하 제거)
```
기존 문제:
- 탐색 알고리즘에서 Node 객체와 Tuple(string, int) 등을 무분별하게 생성하여 힙 메모리를 차지
- 가비지 컬렉터(GC)가 주기적으로 메모리를 해제하면서 프레임 드랍(Spike) 발생

최적화 적용:
1. struct 활용: 모든 탐색 노드를 `class` 대신 `struct` (값 타입)로 변경하여 힙 할당 방지
2. In-Place 수정: `[ThreadStatic]` 스레드 로컬 버퍼를 활용해 새로운 리스트를 생성하지 않고 기존 리스트를 재사용
3. Span<char> 사용: 문자열 파싱 시 Substring 생성을 피하고 `ReadOnlySpan<char>`로 처리
결과: 알고리즘 동작 중 힙 할당(Allocation)이 0에 수렴하도록 최적화 완료!
```

### 5.2 SQLite 데이터베이스 조회 최적화 (LRU Cache)
```
기존 문제:
- ConcurrentDictionary에 단어가 쌓이다가 5000개를 초과하면 `.Clear()`로 전체 삭제
- 삭제 직후 타이핑 시 다시 DB를 읽어야 해서 심각한 렉(Lag) 발생

최적화 적용:
1. LRU Cache 구현: 가장 오랫동안 사용되지 않은 단어부터 하나씩 밀어내는 `LruCache` 클래스 도입
2. 캐시 히트율 안정화: 캐시가 리셋되지 않고 유지되므로 언제 타이핑해도 즉각적인 반응 속도 확보
3. Cold Start 방어: 앱 실행 직후 사전 로딩 중(약 1~2초)일 때는 스페이스바 대기를 생략하고 
   안내 메시지와 함께 원래 문자를 즉각 출력(Fallback) 하도록 개선
```
